...
definePageMeta({ name: '_projectos' })
const client = useSupabaseClient<Database>()

// const { data: projects, error } = await client.from('projects').select('*')
const { data: projects, error, pending } = await useAsyncData('projects', async () => {
...
<template>
    <div class="flex flex-col md:flex-row md:grow">
        ...
        <section class="flex flex-col gap-5 md:gap-0 md:grow">
            ...
            <div
                class="px-7 py-10 md:p-[calc(100vw*120/1920)] grid grid-cols-[repeat(auto-fill,minmax(17rem,1fr))] md:grid-cols-[repeat(auto-fill,minmax(20rem,1fr))] gap-5 md:gap-10 md:max-h-[calc(100vh-11.5rem)] md:overflow-y-auto">
